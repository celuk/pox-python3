...
class management_address (simple_tlv):
  ...
  def _parse_data (self, data):
    asl = data[0] - 1 #ord(data[0]) - 1
    self.address_subtype = data[1] #ord(data[1])
    self.address = data[2:2+asl]

    self.interface_numbering_subtype = data[2+asl] #ord(data[2+asl])
    self.interface_number = struct.unpack("!L",
                                      data[2+asl+1:2+asl+1+4])[0]
    osl = data[7+asl] #ord(data[7+asl])
