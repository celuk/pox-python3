...
class dhcp(packet_base):
    ...
    def _to_str(self):
        ...
        s += ' magic:'+' '.join(
            ["{0:02x}".format(x) for x in self.magic]) #["{0:02x}".format(ord(x)) for x in self.magic])
...
class DHCPRawOption (DHCPOption):
  ...
  def __repr__ (self):
    data = self.data
    if not all(c<127 and c in string.printable for c in data): #if not all(ord(c)<127 and c in string.printable for c in data):
      data = b" ".join("%02x" % (x,) for x in data) #" ".join("%02x" % (ord(x),) for x in data)
    else:
      data = b"".join(x if x >= 32 else "." for x in data) #data = "".join(x if ord(x) >= 32 else "." for x in data)
    if len(data) > 30:
      data = data[:30] + "..."
    n = self._name
...
class DHCPParameterRequestOption (DHCPOption):
  ...
  def unpack (cls, data, code = None):
    self = cls()
    self.options = [x for x in data] #[ord(x) for x in data]
    return self
